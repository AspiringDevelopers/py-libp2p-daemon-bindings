...
class DaemonConnector:
    ...
    async def open_connection(self) -> anyio.abc.SocketStream:
        ...
        if proto_code == protocols.P_UNIX:
            ...
            return await anyio.connect_tcp(remote_host=host, remote_port=port)
        else:
            raise ValueError(
...
class ControlClient:
    ...
    async def _serve_listener(self) -> None:
        try:
            await self.listener.serve(self._dispatcher)
        except Exception:
            pass
    ...
    async def listen(self) -> AsyncIterator["ControlClient"]:
        ...
        if proto_code == protocols.P_UNIX:
            ...
            self.listener = await anyio.create_unix_listener(listen_path)
        elif proto_code == protocols.P_IP4:
            ...
            self.listener = await anyio.create_tcp_listener(
                local_port=port, local_host=host
            )
        else:
            raise ValueError(
                f"protocol not supported: protocol={protocols.protocol_with_code(proto_code)}"
            )
        async with self.listener, anyio.create_task_group() as task_group:
            self.task_group = task_group
            task_group.start_soon(ControlClient._serve_listener, self)
            self.logger.info(
